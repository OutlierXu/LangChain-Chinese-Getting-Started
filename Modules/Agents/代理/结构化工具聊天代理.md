# Structured Tool Chat Agent 结构化工具聊天代理


这个笔记记录一个能够使用多输入工具的聊天代理。

较旧的代理配置为将操作输入指定为单个字符串，但此代理可以使用提供的工具的 `args_schema` 来填充操作输入。
此功能在（`structured-chat-zero-shot-react-description` 或 `AgentType.STRUCTURED_CHAT_ZERO_SHOT_REACT_DESCRIPTION`）中原生可用。
