...
LangChain为LLM提供流式响应支持。目前，我们支持`OpenAI`、`ChatOpenAI`和`ChatAnthropic`实现的流，但对其他LLM实现的流支持正在规划中。若要利用流，请使用 [`CallbackHandler`](https://github.com/hwchase17/langchain/blob/master/langchain/callbacks/base.py)实现`on_llm_new_token`方法。在这个例子中，我们使用`StreamingStdOutCallbackHandler`。
...
如果使用`generate`，我们仍然可以获取`LLMResult`。但是，当前`token_usage`不支持流式传输。
...
下面是一个聊天模型`ChatAnthropic`实现的例子，它使用了他们的`claude`模型。
